...
def forecast():
    ...
    def create_features(df):
        ...
        df = df.copy().sort_values(["depot", "date"]).reset_index(drop=True)

        df["depot_mean_14"] = (
            df.groupby("depot")["passengers_per_day"]
    ...
    if predict_clicked and selected_depot:
        ...
        df_pred_fe = create_features(df_pred)

        # FILTER ONLY REQUIRED DEPOT FOR INFERENCE
        df_pred_depot = df_pred_fe[
            df_pred_fe["depot"] == selected_depot
        ].reset_index(drop=True)


        #df_train_enc, df_pred_enc, encoders = label_encode(df_train_depot, df_pred_depot)
        df_train_enc, df_pred_enc, encoders = label_encode(df_train_fe, df_pred_fe)
        ...
        features = [
            ...
            # depot remains in features automatically
        ...
        model_path = os.path.join(model_dir, "global_passenger_model.pkl")
        ...
        if os.path.exists(model_path):
            ...
            base = XGBRegressor(
                ...
                random_state=42, tree_method="hist", missing=-1
            )
        ...
        **<span style='font-size:24px;color:#D35400'><b>{tomorrow_pred:,.0f}</b></span> passengers**
        ...
        encoded_depot = encoders["depot"].transform([selected_depot])[0]

        df_plot = df_pred_enc[
            df_pred_enc["depot"] == encoded_depot
        ].tail(60).copy()
