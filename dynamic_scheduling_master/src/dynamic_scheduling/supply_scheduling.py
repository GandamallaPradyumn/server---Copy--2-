...
def clean_service_master(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Schedule weight per service (float): fractional services (e.g. 0.5 for an
    # alternate-day service) consume less than a full bus. Default 1.0 when absent
    # — keeps the engine identical to its count-based behaviour.
...
def load_depot_target_or(
    ...
    for _, row in dm.iterrows():
        ...
        try:
            ...
            result[depot] = fallback_or
    return result
def load_depot_fleet(path: Path = DEPOT_FLEET_PATH) -> pd.DataFrame:
    """Load depot fleet master.

    Normalizes headers ``Depot,Product,Code,Fleet`` →
    ``depot,product_name,product_code,count``.  Strips whitespace,
    uppercases ``depot`` and ``product_code``, and coerces ``count`` to int.
    Returns an empty DataFrame when the file is missing.
    """
    if not path.exists():
        return pd.DataFrame(columns=["depot", "product_name", "product_code", "count"])
    df = pd.read_csv(path, encoding="utf-8-sig")
    df.columns = df.columns.str.lower().str.strip()
    rename = {"product": "product_name", "code": "product_code", "fleet": "count"}
    df = df.rename(columns={k: v for k, v in rename.items() if k in df.columns})
    for col in ("depot", "product_code"):
        if col in df.columns:
...
def load_product_master(path: Path = PRODUCT_MASTER_PATH) -> dict[str, float]:
    """Load per-product OR thresholds.

    Normalizes headers ``Product_Name,Product_Code,OR_Thresold`` (typo
    preserved in source data) → ``{product_code: or_threshold_fraction}``.
    Values like ``"90%"`` are converted to ``0.90`` using the same
    ``>1 → /100`` convention as :func:`load_depot_target_or`.  Returns an
    ...
    df = pd.read_csv(path, encoding="utf-8-sig")
    df.columns = df.columns.str.lower().str.strip()
    # Tolerate both "or_thresold" (file typo) and "or_threshold"
    if "or_threshold" not in df.columns and "or_thresold" in df.columns:
        df = df.rename(columns={"or_thresold": "or_threshold"})
    if "product_code" not in df.columns or "or_threshold" not in df.columns:
        return {}
    out: dict[str, float] = {}
    for _, row in df.iterrows():
        code = str(row["product_code"]).strip().upper()
        raw = row["or_threshold"]
        if pd.isna(raw):
            continue
        raw_str = str(raw).strip().rstrip("%")
        try:
...
def compute_depot_pkm_90d_avg(
    daily_ops: pd.DataFrame,
    depot: str,
    target_date: date,
    window_days: int = 90,
) -> float:
    """Mean daily passenger_kms for a depot over the lookback window.

    Returns 0.0 when no rows match (caller decides fallback).
    """
    depot_ops = daily_ops[daily_ops["depot"] == depot]
    if "date" not in depot_ops.columns or "passenger_kms" not in depot_ops.columns:
        return 0.0
    if not pd.api.types.is_datetime64_any_dtype(depot_ops["date"]):
        return 0.0
    end = pd.Timestamp(target_date)
    start = end - pd.Timedelta(days=window_days)
    window = depot_ops[(depot_ops["date"] >= start) & (depot_ops["date"] < end)]
    if len(window) == 0:
        return 0.0
    daily_totals = window.groupby(window["date"].dt.date)["passenger_kms"].sum()
    if len(daily_totals) == 0:
        return 0.0
    return float(daily_totals.mean())


def compute_maintenance_pct(
    predicted_depot_pkm: float,
    pkm_90d_avg: float,
    epk_policy: dict,
    target_date: date | None = None,
) -> float:
    """Pick maintenance % from band based on predicted vs 90-day avg ratio.

    Monday override: if *target_date* is a Monday, returns the
    workshop-holiday rate regardless of demand band or 90d history — the
    workshop is closed on Mondays so no buses are taken in for maintenance.
    """
    if target_date is not None and target_date.weekday() == 0:
        return float(epk_policy.get(
            "maintenance_pct_workshop_holiday",
            epk_policy.get("maintenance_pct_high_band", 0.02),
        ))
    if pkm_90d_avg <= 0:
        return float(epk_policy.get("maintenance_pct_default", 0.08))
    ratio = predicted_depot_pkm / pkm_90d_avg
    if ratio > epk_policy["pkm_band_high_ratio"]:
        return float(epk_policy["maintenance_pct_high_band"])
    if ratio < epk_policy["pkm_band_low_ratio"]:
        return float(epk_policy["maintenance_pct_low_band"])
    return float(epk_policy["maintenance_pct_mid_band"])


def compute_fleet_breakdown(
    base: pd.DataFrame,
    fleet_df: pd.DataFrame,
    depot: str,
    maintenance_pct: float,
    cut_counts_by_product: dict,
    planned_fleet_buffer: float = 0.10,
) -> dict:
    """Per-product fleet accounting for a depot.

    When a ``(depot, product)`` row is missing from ``fleet_df`` the
    available fleet is estimated as ``ceil((1 + planned_fleet_buffer) * planned)``
    — i.e. assume the depot owns ~10% more buses than the services it runs.
    ``estimated`` is set to ``True`` in the output for those rows so the
    dashboard can flag the inference.

    Returns ``{product_code: {available, planned, cut, maintenance, spare, estimated}}``.
    ``spare`` is the raw signed value (can be negative); caller clips for
    allocation but keeps the signed value for diagnostics.
    """
    import math
    depot_upper = str(depot).strip().upper()
    if "product" not in base.columns:
        return {}
    ...
    if fleet_df is None or len(fleet_df) == 0:
        depot_fleet = pd.DataFrame(columns=["product_code", "count"])
    else:
        depot_fleet = fleet_df[fleet_df["depot"] == depot_upper]
    available_by_product = (
        dict(zip(depot_fleet["product_code"], depot_fleet["count"]))
        if len(depot_fleet) else {}
    )

    out: dict[str, dict] = {}
    for product, planned in planned_by_product.items():
        prod_key = str(product).strip().upper()
        planned = float(planned)
        if prod_key in available_by_product:
            available = int(available_by_product[prod_key])
            estimated = False
        else:
            available = int(math.ceil((1.0 + planned_fleet_buffer) * planned))
            estimated = True
        cut = float(cut_counts_by_product.get(product, 0))
        maintenance = int(round(maintenance_pct * available))
        spare = available - maintenance - planned + cut
        out[product] = {
            "available": available,
            "planned": planned,
            "cut": cut,
...
def _dep_time_to_minutes(dep_time_val) -> int | None:
    t = parse_time_safe(dep_time_val)
    if t is None:
        return None
...
def find_consecutive_pair_candidates(
    base: pd.DataFrame,
    product_or_thresholds: dict,
    max_gap_minutes: int,
    cut_mask: pd.Series,
    depot_target_or: float | None = None, 
) -> pd.DataFrame:
    """Detect consecutive same-route pairs whose averaged forecast OR exceeds
    the per-product threshold.

    Walks each route's services in dep_time order and emits one row per
    qualifying adjacent pair. ``OR`` here is the *forecast* OR column
    ``base["or"]``, not historical.

    Skip rules: either side already CUT, unparseable dep_time, gap >
    max_gap_minutes (midnight-wrap aware), mixed products, or avg pair OR
    not above threshold.  When a product is missing from
    ``product_or_thresholds`` and ``depot_target_or`` is provided, the
    depot-level target OR is used as the fallback threshold (otherwise the
    pair is skipped).
    """
    cols = ["earlier_idx", "later_idx", "route", "product",
            "time_diff_min", "avg_pair_or", "suggested_new_slot", "threshold_used"]
    if "route" not in base.columns or len(base) == 0:
        return pd.DataFrame(columns=cols)

    rows = []
    for route, group in base.groupby("route"):
        if len(group) < 2:
            continue
        g = group.copy()
        g["_min"] = g["dep_time"].apply(_dep_time_to_minutes)
        g = g.dropna(subset=["_min"])
        if len(g) < 2:
            continue
        g = g.sort_values("_min")
        idx_list = g.index.tolist()
        for i in range(len(idx_list) - 1):
            i_earlier = idx_list[i]
            i_later = idx_list[i + 1]
            if bool(cut_mask.get(i_earlier, False)) or bool(cut_mask.get(i_later, False)):
                continue
            p_earlier = base.loc[i_earlier, "product"]
            p_later = base.loc[i_later, "product"]
            if p_earlier != p_later:
                continue
            prod_key = str(p_earlier).strip().upper()
            if prod_key in product_or_thresholds:
                threshold = product_or_thresholds[prod_key]
            elif depot_target_or is not None:
                threshold = depot_target_or
            else:
                continue
            earlier_min = g.loc[i_earlier, "_min"]
            later_min = g.loc[i_later, "_min"]
            gap = (later_min - earlier_min) % 1440  # midnight-wrap aware
            if gap == 0 or gap > max_gap_minutes:
                continue
            or_earlier = float(base.loc[i_earlier, "or"])
            or_later = float(base.loc[i_later, "or"])
            avg_or = (or_earlier + or_later) / 2.0
            if avg_or <= threshold:
                continue
            mid_min = (earlier_min + gap // 2) % 1440
            suggested = time(int(mid_min) // 60, int(mid_min) % 60)
            rows.append({
                "earlier_idx": i_earlier,
                "later_idx": i_later,
                "route": route,
                "product": p_earlier,
                "time_diff_min": float(gap),
                "avg_pair_or": avg_or,
                "suggested_new_slot": suggested,
                "threshold_used": float(threshold),
            })
    return pd.DataFrame(rows, columns=cols)


def allocate_adds_with_spare(
    candidates: pd.DataFrame,
    spare_by_product: dict,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Rank candidates by avg_pair_or DESC and allocate against per-product spare.

    Dedupes on ``earlier_idx`` (keeps the highest avg_pair_or pair) so a
    single service is never tagged ADD_SLOT twice. Returns
    ``(allocated_df, no_spare_df)``.
    """
    if candidates is None or len(candidates) == 0:
        empty = pd.DataFrame(columns=candidates.columns if candidates is not None else [])
        return empty, empty
    ranked = candidates.sort_values("avg_pair_or", ascending=False).reset_index(drop=True)
    ranked = ranked.drop_duplicates(subset="earlier_idx", keep="first").reset_index(drop=True)
    remaining = {p: int(c) for p, c in spare_by_product.items()}
    allocated_rows = []
    no_spare_rows = []
    for _, row in ranked.iterrows():
        ...
        if remaining.get(product, 0) > 0:
            ...
            no_spare_rows.append(row)
    allocated = pd.DataFrame(allocated_rows, columns=ranked.columns)
    no_spare = pd.DataFrame(no_spare_rows, columns=ranked.columns)
    return allocated, no_spare
...
def run_epk_policy_engine(
    ...
    product_or_thresholds: dict | None = None,
    depot_target_or: float | None = None,
) -> tuple[pd.DataFrame, dict]:
    ...
    # Step 9: Sequential decisions — CUT first, then fleet capacity, then ADD.
    base["action"] = "NO_CHANGE"
    base["reason"] = "Within policy bounds"
    base["suggested_new_slot"] = None

    # Step 9A — CUT (EPK < epk_discount_cut * CPK, OR below cut threshold, cancellable)
    cut_mask = (
    ...
    if "product" in base.columns and "schedules" in base.columns:
        cut_counts_by_product = (
            base.loc[cut_mask].groupby("product")["schedules"].sum().to_dict()
        )
    elif "product" in base.columns:
        cut_counts_by_product = base.loc[cut_mask].groupby("product").size().to_dict()
    else:
        cut_counts_by_product = {}

    # Step 9B — Fleet capacity per (depot, product)
    pkm_90d_avg = compute_depot_pkm_90d_avg(
        daily_ops, depot, target_date,
        window_days=epk_policy.get("pkm_90d_window", 90),
    )
    maintenance_pct = compute_maintenance_pct(
        predicted_depot_pkm, pkm_90d_avg, epk_policy, target_date=target_date,
    )
    fleet_breakdown = compute_fleet_breakdown(
        base,
        fleet_df if fleet_df is not None else pd.DataFrame(),
        depot, maintenance_pct, cut_counts_by_product,
    )
    spare_by_product = {p: max(0, v["spare"]) for p, v in fleet_breakdown.items()}

    # Step 9C — ADD via consecutive-pair detection, capped by SPARE
    candidates = find_consecutive_pair_candidates(
        base,
        product_or_thresholds or {},
        max_gap_minutes=epk_policy.get("pair_max_gap_minutes", 30),
        cut_mask=cut_mask,
        depot_target_or=depot_target_or,
    )
    ...
    added_by_product = (
        allocated.groupby("product").size().to_dict() if len(allocated) else {}
    )
    no_spare_by_product = (
        no_spare.groupby("product").size().to_dict() if len(no_spare) else {}
    )
    for p in fleet_breakdown:
        added = int(added_by_product.get(p, 0))
        unfilled = int(no_spare_by_product.get(p, 0))
        fleet_breakdown[p]["added"] = added
        fleet_breakdown[p]["to_add"] = added + unfilled
    ...
    out_cols += [
        "product", "schedules", "dep_time", "allocated_pkm", "planned_kms", "revenue",
    ...
    action_order = pd.Categorical(
        out["action"],
        categories=["ADD_SLOT", "ADD_CANDIDATE_NO_SPARE", "CUT", "NO_CHANGE"],
        ordered=True,
    )
    out["_action_rank"] = action_order
    ...
    summary = {
        ...
        "count_add_candidates_no_spare": int(
            (base["action"] == "ADD_CANDIDATE_NO_SPARE").sum()
        ),
...
def run_all_depots_epk(
    ...
    for depot, predicted_pkm in depot_predictions.items():
        schedule_df, summary = run_epk_policy_engine(
            ...
            fleet_df=fleet_df,                          # ✅ ADD THIS
            product_or_thresholds=product_or_thresholds,
            depot_target_or=depot_target_or_map.get(depot),
        )
        if "error" in summary:
            continue
