...
from auth import get_depot_settings, get_connection
...
def RM_sheet(user_region, role):
    ...
    if selection == "INDIVIDUAL DEPOTS":
        ...
        cur.execute("SELECT depot_name FROM TS_ADMIN WHERE region=%s ORDER BY depot_name",(user_region,))
        rm_depots = [r[0] for r in cur.fetchall()]
        ...
        def get_latest_date(depot_name):
            try:
                conn = get_connection()
                cur = conn.cursor()
                cur.execute("SELECT MAX(data_date) FROM input_data WHERE depot_name=%s", (depot_name,))
        ...
        def fetch_existing_data_for_dates(depot_name, date_columns):
            try:
                ...
                cols = ", ".join(config_data["db_columns"])
                placeholders = ','.join(['%s']*len(date_columns))
                q = f"SELECT {cols} FROM input_data WHERE depot_name=%s AND data_date IN ({placeholders})"
                cursor.execute(q,[depot_name]+date_columns)
        ...
        # --- fetch all depots for region ---
            ...
            def get_latest_region_date(dep_list):
                try:
                    ...
                    cur.execute(f"SELECT MAX(data_date) FROM input_data WHERE depot_name IN ({placeholders})", dep_list)
                    latest = cur.fetchone()[0]
                    cur.close(); conn.close()
                    return latest
                except:
            ...
            def fetch_region_data(dep_list, date_columns):
                try:
                    ...
                    cursor = conn.cursor(dictionary=True)
                    cols = ", ".join(config_data["db_columns"])
                    placeholders_depots = ",".join(["%s"]*len(dep_list))
                    placeholders_dates = ",".join(["%s"]*len(date_columns))
                    q = f"""
                        SELECT depot_name, data_date, {cols}
                        FROM input_data
                        WHERE depot_name IN ({placeholders_depots})
                        AND data_date IN ({placeholders_dates})
                    """
                    cursor.execute(q, dep_list + date_columns)
