...
# SERVICE_OPS_RAW_PATH — previously a CSV fallback; now served from MySQL via data_pipeline.load_service_raw_master()
...
def run_supply_scheduling(
    ...
    # Load daily ops — prefer gold parquet, fall back to MySQL raw table
    if SERVICE_OPS_GOLD_PATH.exists():
        daily_ops_raw = pd.read_parquet(SERVICE_OPS_GOLD_PATH)
    else:
        # Fall back to MySQL raw table (replaces old CSV fallback)
        try:
            from data_pipeline import load_service_raw_master
            daily_ops_raw = load_service_raw_master()  # full table
        except Exception as _e:
            raise FileNotFoundError(
                f"No service gold parquet found and DB fallback failed: {_e}"
            )
