...
class prod_ratios_DM:
    ...
    def ratios(self):
        ...
        # ✅ Fetch category from TS_ADMIN
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT category FROM TS_ADMIN WHERE depot_name = %s", (selected_depot,))
            result = cursor.fetchone()
            category = result[0].strip().lower() if result else "unknown"
        except mysql.connector.Error as err:
