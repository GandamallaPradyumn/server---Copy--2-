...
def absdmmain():
    ...
    user_role = get_role_by_userid(userid)
    user_depot = get_depot_by_userid(userid)

    if "models" not in st.session_state:
        st.session_state.models = load_saved_models()

    if user_role == "ADMIN":
        if st.button("🧠 Train / Retrain Models", key="train_models_btn"):
    ...
    with tab1:
        ...
        if user_role == "ADMIN":
            depot_sel = st.selectbox(
                "Select Depot",
                sorted(df_eng["depot_name"].unique()),
                key="tab1depot"
            )
        else:
            depot_sel = user_depot
            st.info(f"📍 Depot: **{depot_sel}**")
        ...
        # ================= TODAY =================
        with colA:
            if st.button(f"Predict Today ({today})", key="predict_today_btn"):
        ...
        # ================= TOMORROW =================
        with colB:
            if st.button(f"Predict Tomorrow ({tomorrow})", key="predict_tomorrow_btn"):
    ...
    with tab2:
        ...
        if user_role == "ADMIN":
            ...
            depot = user_depot
            st.info(f"📍 Depot: **{depot}**")


        if st.button("Generate 7-Day Forecast", key="forecast_7day_btn"):
...
# =====================================================
    with tab3:
        st.header("📊 Variance (Past 15 Days)")
